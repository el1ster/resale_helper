...
def generate_receipt_image(snapshot: dict, final_price: float) -> io.BytesIO:
    ...
    try:
        # Намагаємось використати стандартні шрифти Windows/Linux
    ...
    draw.line((50, 110, 650, 110), fill=(100, 100, 100), width=2)
    ...
    draw.text((250, y), f"{snapshot.get('category_name', 'Невідомо')}", fill=(255, 255, 255), font=font_bold)
    ...
    draw.line((50, y+50, 650, y+50), fill=(100, 100, 100), width=1)
    ...
    for label, name, mult in factors:
        if name and mult is not None:
            ...
            # Значення
            draw.text((250, y), str(name)[:25] + ("..." if len(str(name)) > 25 else ""), fill=(255, 255, 255), font=font_text)
            # Множник
            mult_str, color = format_multiplier(mult)
            draw.text((580, y), mult_str, fill=color, font=font_bold)
            y += line_height

    draw.line((50, y+30, 650, y+30), fill=(100, 100, 100), width=2)
